...
# Train windows shuffle (time split korunur; sadece train içi örnek sırası karışır)
...
def transform_minmax(X: np.ndarray, mins: np.ndarray, maxs: np.ndarray) -> np.ndarray:
    denom = (maxs - mins)
    denom = np.where(denom < EPS, 1.0, denom)  # sabit kolon varsa bölme hatası olmasın
    return (X - mins) / denom


def make_ts_dataset(X_scaled: np.ndarray, y: np.ndarray, lookback: int, batch_size: int, shuffle: bool) -> tf.data.Dataset:
    """
    timeseries_dataset_from_array hedef hizalaması:
      sequence i -> X[i : i+lookback]
      target i   -> targets[i]
    Bu yüzden targets'ı y[lookback-1:] olarak kaydırıyoruz:
      target i = y[i+lookback-1]
    """
    if len(X_scaled) < lookback:
        raise ValueError(f"Segment çok kısa: len={len(X_scaled)} < lookback={lookback}")

    targets = y[lookback - 1:]                      # (N-lookback+1,)
    data = X_scaled                                 # (N, d)
    end_index = len(data) - lookback                # son başlangıç indexi
...
def build_lstm(lookback: int, n_features: int) -> tf.keras.Model:
    model = Sequential([
        LSTM(64, activation="tanh", input_shape=(lookback, n_features)),
        Dropout(0.2),
...
def main():
    ...
    # Train içinde val ayır (time-based)
    ...
    # timeseries datasets (strict split: val/test window'ları kendi segmenti içinde)
    ...
    # class_weight (sequence targetları y[lookback-1:])
