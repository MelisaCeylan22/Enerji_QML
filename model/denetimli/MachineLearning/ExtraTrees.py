...
OUT_DIR = DATA_DIR / "ml_out" / "extra_trees"
...
# Spyder bazen n_jobs=-1 ile takılabiliyor.
# Takılma olursa N_JOBS=1 yap.
N_JOBS = -1

# ExtraTrees hiperparametreleri (baseline)
ET_PARAMS = dict(
    n_estimators=600,
    max_depth=None,
    min_samples_split=2,
    min_samples_leaf=1,
    max_features="sqrt",
    bootstrap=False,              # ExtraTrees genelde bootstrap=False iyi baseline
    n_jobs=N_JOBS,
    random_state=RANDOM_STATE,
    class_weight="balanced",      # dengesiz sınıf için iyi baseline
    verbose=0,                    # takılma/ilerleme görmek istersen 2 yap
)
