...
# train threshold (p99) sadece train'den!
p99_gap = np.percentile(train_df["Global_active_power"].values, 99)

# baseline: testte GAP > p99 ise anomali
y_base = (test_df["Global_active_power"].values > p99_gap).astype(int)
y_true = test_df["label"].values.astype(int)

tn, fp, fn, tp = confusion_matrix(y_true, y_base).ravel()
acc  = accuracy_score(y_true, y_base)
prec = precision_score(y_true, y_base, zero_division=0)
rec  = recall_score(y_true, y_base, zero_division=0)
f1   = f1_score(y_true, y_base, zero_division=0)

print("=== BASELINE (GAP > p99_train) ===")
print(f"TP={tp}, TN={tn}, FP={fp}, FN={fn}")
print(f"Accuracy={acc:.4f} | Precision={prec:.4f} | Recall={rec:.4f} | F1={f1:.4f}")
print("p99_gap(train) =", p99_gap)